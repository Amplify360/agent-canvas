    ...
    <div className="dock-view">
      ...
      <div className="dock-view__track">
        {agents.map((agent, index) => {
          ...
          const statusConfig = getAgentStatusConfig(coreFields.status);
          ...
          return (
            ...
            >
              ...
              <div className="dock-item__tools-compact">
                {coreFields.tools.slice(0, 3).map((tool: string) => {
      ...
      {selectedAgent && (() => {
        const selectedCoreFields = getAgentCoreFields(selectedAgent);
        const selectedStatusConfig = getAgentStatusConfig(selectedCoreFields.status);
        return (
        <div className="dock-carousel">
          ...
          <div className="dock-carousel__card">
            ...
            {selectedCoreFields.objective && (
              <p className="dock-carousel__objective">{selectedCoreFields.objective}</p>
            )}

            <div className="dock-carousel__tools">
              {selectedCoreFields.tools.map((tool: string) => {
                const toolDisplay = getToolDisplay(tool);
                return (
                  <span
                    key={tool}
